        ...
        return {
            ...
            putOne: function(schema, example) {
                ...
                function insert(schema, example) {
                    var name = schema.name,
                        insert_id = SQL.getScalar('SELECT ' + sequenceName(name, schema.primaryKey) + '.nextval from dual'),
            ...
            getFromTable: function(name) {
                ...
                rows.each(function(row) {
                    switch(row.DATA_TYPE) {
                        case 'NUMBER':
                            if (SQL.getScalar('SELECT COUNT(*) FROM user_sequences WHERE sequence_name=' + SQL.quote(sequenceName(name, row.COLUMN_NAME)))) {
                                fields.push({
