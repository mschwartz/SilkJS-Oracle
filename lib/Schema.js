...
/*global exports, SQL, require, global, req */
    ...
    var Schema = function() {
        ...
        return {
            ...
            getFromTable: function(name) {
                ...
                var fields = [];
                var rows = SQL.getDataRows('SELECT column_name, data_type, data_length, data_precision, data_scale FROM user_tab_columns WHERE table_name='+SQL.quote(name));
...
console.dir({ schema: schema, existing: existing });
                ...
                if (existing.primaryKey && !schema.primaryKey) {
                    ...
                    SQL.update([
                        ...
                        '   "' + name + '"',
                        'ADD',
                        '   CONSTRAINT ' + schema.primaryKey  + '_pk ("' + schema.primaryKey + '")'
                    ]);
                }
                else if (schema.primaryKey !== existing.primaryKey) {
					SQL.update([
